class DecisionsController < ApplicationController
  ...
  def upvote 
    ...
    @decision.upvote_by current_user
    @decision.positive_coefficient += current_user.apartment.coefficient
    @decision.save
  ...
  def downvote
    @decision = Decision.find(params[:id])
    @decision.downvote_by current_user
    @decision.negative_coefficient += current_user.apartment.coefficient
    @decision.save
    redirect_to decisions_path
  end
  ...
  private
    ...
    def decision_params
      params.require(:decision).permit(:tittle, :description)
    end
end
